...
#include <time.h>
#include <unistd.h> 
#include <fcntl.h>   // For lseek, write
...
#define FCB_COUNT FCB_BLOCKS_COUNT
...
void write_block(void *block, uint32_t block_number) {
    ...
    if (lseek(vdisk_fd, (off_t)offset, SEEK_SET) < 0) {
        perror("lseek failed");
        exit(1);
    }
...
void init_superblock(int total_blocks, int available_blocks, int total_fcbs) {
    ...
    write_block((void *)block, SUPERBLOCK_BLOCK);
}
...
void init_root_directory() {
    ...
    for (int i = 0; i < num_entries; i++) {
        struct DirectoryEntry *dir_entry = (struct DirectoryEntry *)(block + i * dir_entry_size);
